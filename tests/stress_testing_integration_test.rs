...
mod tests {
    use ass_easy_loop::{
        ...
        TestExecutionError, TestParameterError, SystemError,
    ...
    use ass_easy_loop::test_processor::{
        ...
        ValidationCriteria, ResourceLimits, TimingMeasurement
    };
    use heapless::Vec;
    ...
    fn test_stress_test_measurements() {
        ...
        processor.execute_stress_test(stress_params, timestamp).unwrap();
        
        // Update stress test with various measurements
        
        // Normal operation measurements
        processor.update_stress_test_measurements(25, 4096, 800, true, timestamp + 100).unwrap();
        processor.update_stress_test_measurements(30, 4200, 900, true, timestamp + 200).unwrap();
        processor.update_stress_test_measurements(35, 4500, 1100, true, timestamp + 300).unwrap();
        
        // High load measurements
        processor.update_stress_test_measurements(60, 8192, 2500, true, timestamp + 400).unwrap();
        processor.update_stress_test_measurements(75, 10240, 3200, false, timestamp + 500).unwrap(); // Failed operation
        
        // Performance degradation measurements
        processor.update_stress_test_measurements(90, 12288, 8000, false, timestamp + 600).unwrap(); // High latency
        processor.update_stress_test_measurements(85, 11000, 6500, true, timestamp + 700).unwrap();
    ...
    fn test_performance_degradation_detection() {
        ...
        for i in 0..20 {
            ...
            response_time += 200; // Increase by 0.2ms each iteration
            let cpu_usage = 20 + (i * 2); // Gradually increase CPU usage
    ...
    fn test_stress_test_timeout_and_completion() {
        ...
        let test_id = processor.execute_stress_test(stress_params, start_timestamp).unwrap();
        ...
        processor.update_stress_test_measurements(55, 7168, 1400, true, start_timestamp + 1000).unwrap();
        processor.update_stress_test_measurements(50, 6656, 1100, true, start_timestamp + 1500).unwrap();

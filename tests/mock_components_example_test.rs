...
fn test_mock_usb_device_connection() -> TestResult {
    ...
    assert_no_std!(!device.is_configured());
    
    // Test connection
    device.connect();
    assert_no_std!(device.is_connected());
    assert_no_std!(!device.is_configured()); // Should not be configured yet
...
fn test_mock_usb_device_communication() -> TestResult {
    ...
    assert_no_std!(device.receive_report(&host_data, 1100).is_ok());
    assert_eq_no_std!(device.received_report_count(), 1);
    
    if let Some(report) = device.get_received_report() {
        assert_eq_no_std!(report.data.as_slice(), &host_data);
        assert_eq_no_std!(report.timestamp_ms, 1100);
    } else {
...
fn test_mock_usb_device_error_injection() -> TestResult {
    ...
    assert_no_std!(device.send_report(&test_data, 1100).is_ok());
...
fn test_mock_system_state_basic() -> TestResult {
    ...
    // Test initial state
    assert_eq_no_std!(state.get_uptime_ms(), 0);
    assert_eq_no_std!(state.get_battery_voltage(), 3300);
    assert_no_std!(!state.is_pemf_active());
    
    // Test uptime update
    state.set_uptime(5000);
    assert_eq_no_std!(state.get_uptime_ms(), 5000);
    
    // Test battery state changes
    state.set_battery_state(BatteryState::Low, 2900);
    assert_eq_no_std!(state.get_battery_voltage(), 2900);
    
    let health = state.get_system_health();
    assert_eq_no_std!(health.battery_state, BatteryState::Low);
    assert_eq_no_std!(health.battery_voltage_mv, 2900);
...
fn test_mock_system_state_pemf_control() -> TestResult {
    ...
    let health = state.get_system_health();
    assert_no_std!(health.pemf_active);
    assert_eq_no_std!(health.pemf_cycle_count, 1);
    
    // Test multiple activations increment cycle count
    state.set_pemf_active(true);
    let health2 = state.get_system_health();
    assert_eq_no_std!(health2.pemf_cycle_count, 2);
...
fn test_mock_system_state_error_tracking() -> TestResult {
    ...
    let health = state.get_system_health();
    assert_eq_no_std!(health.error_counts.total_error_count, 0);
    assert_eq_no_std!(health.error_counts.adc_read_errors, 0);
    
    // Test error increment
    state.increment_error("adc");
    let health2 = state.get_system_health();
    assert_eq_no_std!(health2.error_counts.adc_read_errors, 1);
    assert_eq_no_std!(health2.error_counts.total_error_count, 1);
    
    // Test different error types
    state.increment_error("usb");
    state.increment_error("gpio");
    let health3 = state.get_system_health();
    assert_eq_no_std!(health3.error_counts.usb_transmission_errors, 1);
    assert_eq_no_std!(health3.error_counts.gpio_operation_errors, 1);
    assert_eq_no_std!(health3.error_counts.total_error_count, 3);
...
fn test_mock_bootloader_hardware_state() -> TestResult {
    ...
    assert_no_std!(!state.mosfet_state);
    assert_no_std!(!state.pemf_pulse_active);
    ...
    assert_no_std!(current_state.mosfet_state);
    assert_no_std!(current_state.pemf_pulse_active);
    assert_no_std!(!current_state.is_safe_for_bootloader());
...
fn test_mock_bootloader_task_management() -> TestResult {
    ...
    assert_eq_no_std!(hardware.get_task_status(TaskPriority::High), TaskShutdownStatus::Running);
    assert_eq_no_std!(hardware.get_task_status(TaskPriority::Medium), TaskShutdownStatus::Running);
    assert_eq_no_std!(hardware.get_task_status(TaskPriority::Low), TaskShutdownStatus::Running);
    
    // Test task shutdown progression
    hardware.set_task_status(TaskPriority::High, TaskShutdownStatus::ShutdownRequested);
    assert_eq_no_std!(hardware.get_task_status(TaskPriority::High), TaskShutdownStatus::ShutdownRequested);
    
    hardware.complete_task_shutdown(TaskPriority::High);
    assert_eq_no_std!(hardware.get_task_status(TaskPriority::High), TaskShutdownStatus::ShutdownComplete);
...
fn test_data_generator_basic() -> TestResult {
    ...
    assert_no_std!(u16_val <= u16::MAX);
    assert_no_std!(u8_val <= u8::MAX);
    assert_no_std!(bool_val == true || bool_val == false);
...
fn test_data_generator_ranges() -> TestResult {
    ...
    for _ in 0..10 {
        ...
        assert_no_std!(val >= 10 && val < 20);
    }
    
    // Test battery-specific generators
    let battery_adc = gen.generate_battery_adc();
    assert_no_std!(battery_adc >= 1000 && battery_adc < 2000);
    
    let battery_voltage = gen.generate_battery_voltage();
    assert_no_std!(battery_voltage >= 2800 && battery_voltage < 4200);
    
    let temperature = gen.generate_temperature();
    assert_no_std!(temperature >= -10 && temperature < 60);
...
fn test_validator_float_comparison() -> TestResult {
    ...
    assert_no_std!(validator.validate_float_approx(100.0, 100.5));
    
    // Test values outside tolerance
    assert_no_std!(!validator.validate_float_approx(1.0, 1.02));
    assert_no_std!(!validator.validate_float_approx(100.0, 102.0));
    
    // Test zero handling
    assert_no_std!(validator.validate_float_approx(0.0, 0.0005));
    assert_no_std!(!validator.validate_float_approx(0.0, 0.01));
...
fn test_validator_range_validation() -> TestResult {
    ...
    assert_no_std!(validator.validate_range(5, 1, 10));
    assert_no_std!(validator.validate_range(1, 1, 10)); // Edge case: min value
    assert_no_std!(validator.validate_range(10, 1, 10)); // Edge case: max value
    assert_no_std!(!validator.validate_range(0, 1, 10)); // Below range
    assert_no_std!(!validator.validate_range(11, 1, 10)); // Above range
    
    // Test domain-specific validations
    assert_no_std!(validator.validate_battery_voltage(3300));
    assert_no_std!(!validator.validate_battery_voltage(1000)); // Too low
    assert_no_std!(!validator.validate_battery_voltage(5000)); // Too high
    
    assert_no_std!(validator.validate_adc_reading(2048));
    assert_no_std!(!validator.validate_adc_reading(5000)); // Above 12-bit range
    
    assert_no_std!(validator.validate_temperature(25));
    assert_no_std!(!validator.validate_temperature(-50)); // Too cold
    assert_no_std!(!validator.validate_temperature(100)); // Too hot
...
fn test_mock_test_environment_basic() -> TestResult {
    ...
    // Test initial state
    assert_no_std!(env.usb_device.is_connected());
    assert_no_std!(env.usb_device.is_configured());
    assert_eq_no_std!(env.get_current_time(), 0);
    
    // Test time advancement
    env.advance_time(1000);
    assert_eq_no_std!(env.get_current_time(), 1000);
    assert_eq_no_std!(env.system_state.get_uptime_ms(), 1000);
...
fn test_mock_test_environment_pemf_simulation() -> TestResult {
    ...
    assert_no_std!(final_cycles > initial_cycles);
    
    // Verify hardware state is safe after cycle
    let hw_state = env.bootloader_hardware.get_hardware_state();
    assert_no_std!(!hw_state.pemf_pulse_active);
    assert_no_std!(!hw_state.mosfet_state);
...
fn test_mock_test_environment_usb_simulation() -> TestResult {
    ...
    assert_eq_no_std!(env.usb_device.sent_report_count(), 5);
    
    // Verify reports were generated
    for i in 0..5 {
        if let Some(report) = env.usb_device.get_sent_report() {
            assert_eq_no_std!(report.data.len(), 32);
            assert_eq_no_std!(report.timestamp_ms, i as u32);
        } else {
...
fn test_mock_test_environment_error_simulation() -> TestResult {
    ...
    assert_eq_no_std!(final_errors, initial_errors + 2);
...
fn test_helper_functions() -> TestResult {
    ...
    assert_no_std!(usb_env.usb_device.is_connected());
    assert_no_std!(usb_env.usb_device.is_configured());
    
    // Test error test environment helper
    let error_env = test_helpers::create_error_test_env();
    assert_no_std!(error_env.usb_device.is_connected());
    ...
    assert_no_std!(hw_state.is_safe_for_bootloader());
    
    // Test validation helper
    assert_no_std!(test_helpers::validate_test_env(&usb_env).is_ok());
...
fn test_mock_macros() -> TestResult {
    ...
    let test_data = [0x01, 0x02, 0x03, 0x04];
    assert_no_std!(env.usb_device.send_report(&test_data, 1000).is_ok());

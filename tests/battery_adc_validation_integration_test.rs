...
use ass_easy_loop::test_processor::{
    BatteryAdcParameters, AdcValidationMode, BatteryAdcMeasurements, 
    BatteryAdcTestResult, TestCommandProcessor, BatteryState
};
use ass_easy_loop::{TestType, TestStatus, TestExecutionError};
...
fn test_battery_adc_parameter_creation() {
    ...
    assert_eq!(parsed.reference_voltage_mv, default_params.reference_voltage_mv);
    ...
    assert_eq!(parsed.calibration_enabled, default_params.calibration_enabled);
    assert_eq!(parsed.state_transition_test, default_params.state_transition_test);
...
fn test_battery_state_transitions() {
    ...
    assert!(!BatteryState::Charging.is_valid_transition(BatteryState::Low, 1700)); // ADC too high for Low
}
...
fn test_battery_adc_test_result() {
    ...
    assert_eq!(response.command_type, 0x83); // TestResponse::TestResult
...
fn test_battery_adc_test_parameter_conversion() {
    ...
    assert_eq!(test_params.sample_rate_hz, 10); // 40 samples / 4 seconds = 10 Hz
    assert_eq!(test_params.validation_criteria.min_success_rate_percent, 97); // 100 - 2.5 = 97.5 -> 97
    assert_eq!(test_params.resource_limits.max_cpu_usage_percent, 15);
    assert_eq!(test_params.resource_limits.max_memory_usage_bytes, 1024);
    ...
    assert_eq!(parsed_adc_params.test_duration_ms, adc_params.test_duration_ms);
    assert_eq!(parsed_adc_params.reference_voltage_mv, adc_params.reference_voltage_mv);
    assert_eq!(parsed_adc_params.tolerance_percent, adc_params.tolerance_percent);
    assert_eq!(parsed_adc_params.sample_count, adc_params.sample_count);
    assert_eq!(parsed_adc_params.validation_mode, adc_params.validation_mode);
}
...
fn test_adc_validation_modes() {
    ...
    // Test mode parsing
    assert_eq!(AdcValidationMode::from_u8(0x00), Some(AdcValidationMode::Accuracy));
    assert_eq!(AdcValidationMode::from_u8(0x01), Some(AdcValidationMode::Calibration));
    assert_eq!(AdcValidationMode::from_u8(0x02), Some(AdcValidationMode::StateTransition));
    assert_eq!(AdcValidationMode::from_u8(0x03), Some(AdcValidationMode::Comprehensive));

...
fn test_log_config_creation() {
    ...
    assert_eq_no_std!(default_config.max_level, LogLevel::Info); // Should match compile-time default
    assert_eq_no_std!(default_config.queue_size, 32);
    assert_eq_no_std!(default_config.usb_vid, 0x1234);
    assert_eq_no_std!(default_config.usb_pid, 0x5678);
    
    // Test debug configuration
    let debug_config = LogConfig::debug_config();
    assert_eq_no_std!(debug_config.max_level, LogLevel::Debug);
    assert_eq_no_std!(debug_config.queue_size, 128);
    assert_no_std!(debug_config.enable_battery_logs);
    assert_no_std!(debug_config.enable_pemf_logs);
    assert_no_std!(debug_config.enable_system_logs);
    assert_no_std!(debug_config.enable_usb_logs);
    
    // Test minimal configuration
    let minimal_config = LogConfig::minimal_config();
    assert_eq_no_std!(minimal_config.max_level, LogLevel::Error);
    assert_eq_no_std!(minimal_config.queue_size, 16);
    assert_no_std!(!minimal_config.enable_battery_logs);
    assert_no_std!(!minimal_config.enable_pemf_logs);
    assert_no_std!(!minimal_config.enable_system_logs);
    assert_no_std!(!minimal_config.enable_usb_logs);
}
...
fn test_log_level_filtering() {
    ...
    assert_no_std!(config.should_log(LogLevel::Debug, LogCategory::General));
...
fn test_configuration_validation() {
    ...
    #[cfg(not(debug_assertions))]
    assert_eq_no_std!(config.validate(), Err(ConfigError::InvalidLogLevel));
    #[cfg(debug_assertions)]
    assert_no_std!(config.validate().is_ok());
    ...
    config.usb_vid = 0;
    assert_eq_no_std!(config.validate(), Err(ConfigError::InvalidUsbId));
...
fn test_configuration_setters() {
    ...
    assert_no_std!(config.set_max_level(LogLevel::Warn).is_ok());
    assert_eq_no_std!(config.max_level, LogLevel::Warn);
    ...
    assert_no_std!(config.set_pemf_logs(false).is_ok());
    assert_no_std!(!config.enable_pemf_logs);
    
    assert_no_std!(config.set_system_logs(false).is_ok());
    assert_no_std!(!config.enable_system_logs);
    
    assert_no_std!(config.set_usb_logs(false).is_ok());
    assert_no_std!(!config.enable_usb_logs);
    ...
    // Test timeout setter
    assert_no_std!(config.set_transmission_timeout(200).is_ok());
    assert_eq_no_std!(config.transmission_timeout_ms, 200);
    
    assert_eq_no_std!(config.set_transmission_timeout(0), Err(ConfigError::InvalidTimeout));
    assert_eq_no_std!(config.set_transmission_timeout(15000), Err(ConfigError::InvalidTimeout));
    
    // Test retry attempts setter
    assert_no_std!(config.set_max_retry_attempts(5).is_ok());
    assert_eq_no_std!(config.max_retry_attempts, 5);
    
    assert_eq_no_std!(config.set_max_retry_attempts(0), Err(ConfigError::InvalidRetryCount));
    assert_eq_no_std!(config.set_max_retry_attempts(15), Err(ConfigError::InvalidRetryCount));
}
...
fn test_configuration_serialization() {
    ...
    assert_eq_no_std!(serialized.len(), 16);
    
    // Test deserialization
    let deserialized = LogConfig::deserialize(&serialized).unwrap();
    assert_eq_no_std!(config.max_level, deserialized.max_level);
    assert_eq_no_std!(config.queue_size, deserialized.queue_size);
    assert_eq_no_std!(config.usb_vid, deserialized.usb_vid);
    assert_eq_no_std!(config.usb_pid, deserialized.usb_pid);
    assert_eq_no_std!(config.enable_battery_logs, deserialized.enable_battery_logs);
    assert_eq_no_std!(config.enable_pemf_logs, deserialized.enable_pemf_logs);
    assert_eq_no_std!(config.enable_system_logs, deserialized.enable_system_logs);
    assert_eq_no_std!(config.enable_usb_logs, deserialized.enable_usb_logs);
...
fn test_invalid_serialization() {
    ...
    invalid_data[1] = 0;
    invalid_data[2] = 0;
    assert_eq_no_std!(LogConfig::deserialize(&invalid_data), Err(ConfigError::InvalidQueueSize));
    
    // Invalid USB VID (0)
    invalid_data[1] = 32; // Valid queue size
    invalid_data[3] = 0;
    invalid_data[4] = 0;
    assert_eq_no_std!(LogConfig::deserialize(&invalid_data), Err(ConfigError::InvalidUsbId));
}

#[test]
fn test_usb_control_commands() {
    // Test command conversion from u8
    assert_eq_no_std!(UsbControlCommand::from_u8(0x01), Some(UsbControlCommand::GetConfig));
    assert_eq_no_std!(UsbControlCommand::from_u8(0x02), Some(UsbControlCommand::SetConfig));
    assert_eq_no_std!(UsbControlCommand::from_u8(0x03), Some(UsbControlCommand::SetLogLevel));
    assert_eq_no_std!(UsbControlCommand::from_u8(0x04), Some(UsbControlCommand::EnableCategory));
    assert_eq_no_std!(UsbControlCommand::from_u8(0x05), Some(UsbControlCommand::DisableCategory));
    assert_eq_no_std!(UsbControlCommand::from_u8(0x06), Some(UsbControlCommand::ResetConfig));
    assert_eq_no_std!(UsbControlCommand::from_u8(0x07), Some(UsbControlCommand::GetStats));
    
    // Test invalid command
    assert_eq_no_std!(UsbControlCommand::from_u8(0xFF), None);
    assert_eq_no_std!(UsbControlCommand::from_u8(0x00), None);
}
...
fn test_config_error_messages() {
    ...
    assert_eq_no_std!(ConfigError::InvalidLogLevel.as_str(), "Invalid log level");
    assert_eq_no_std!(ConfigError::CategoryDisabledAtCompileTime.as_str(), "Category disabled at compile time");
}
...
fn test_compile_time_feature_flags() {
    ...
    assert_no_std!(ass_easy_loop::config::logging::ENABLE_BATTERY_LOGS);
    #[cfg(not(feature = "battery-logs"))]
    assert_no_std!(!ass_easy_loop::config::logging::ENABLE_BATTERY_LOGS);
    
    #[cfg(feature = "pemf-logs")]
    assert_no_std!(ass_easy_loop::config::logging::ENABLE_PEMF_LOGS);
    #[cfg(not(feature = "pemf-logs"))]
    assert_no_std!(!ass_easy_loop::config::logging::ENABLE_PEMF_LOGS);
    
    #[cfg(feature = "system-logs")]
    assert_no_std!(ass_easy_loop::config::logging::ENABLE_SYSTEM_LOGS);
    #[cfg(not(feature = "system-logs"))]
    assert_no_std!(!ass_easy_loop::config::logging::ENABLE_SYSTEM_LOGS);
    
    #[cfg(feature = "usb-logs")]
    assert_no_std!(ass_easy_loop::config::logging::ENABLE_USB_LOGS);
...
fn test_configuration_combinations() {
    ...
    assert_eq_no_std!(prod_config.max_level, LogLevel::Error);
    assert_no_std!(!prod_config.enable_pemf_logs);
    assert_no_std!(!prod_config.enable_usb_logs);
    
    // Test development configuration
    let dev_config = LogConfig::debug_config();
    assert_eq_no_std!(dev_config.max_level, LogLevel::Debug);
    assert_no_std!(dev_config.enable_battery_logs);
    assert_no_std!(dev_config.enable_pemf_logs);
    assert_no_std!(dev_config.enable_system_logs);
    assert_no_std!(dev_config.enable_usb_logs);
    ...
    assert_no_std!(selective_config.should_log(LogLevel::Info, LogCategory::General));
    assert_eq_no_std!(selective_config.should_log(LogLevel::Info, LogCategory::Battery),
               selective_config.enable_battery_logs && ass_easy_loop::config::logging::ENABLE_BATTERY_LOGS);
    assert_no_std!(!selective_config.should_log(LogLevel::Info, LogCategory::Pemf));
    assert_eq_no_std!(selective_config.should_log(LogLevel::Info, LogCategory::System),
               selective_config.enable_system_logs && ass_easy_loop::config::logging::ENABLE_SYSTEM_LOGS);
    assert_no_std!(!selective_config.should_log(LogLevel::Info, LogCategory::Usb));
}
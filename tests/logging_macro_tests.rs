...
use ass_easy_loop::config::{ConfigError, LogCategory, LogConfig, UsbControlCommand};
...
fn test_log_config_creation() {
    ...
    assert_eq!(default_config.max_level, LogLevel::Debug); // Should match compile-time default
    ...
    // Check that categories match compile-time availability
...
fn test_category_filtering() {
    ...
    assert_eq!(
        config.should_log(LogLevel::Info, LogCategory::Battery),
    ...
    assert_eq!(
        config.should_log(LogLevel::Info, LogCategory::System),
...
fn test_configuration_setters() {
    ...
    assert_eq!(
        config.set_transmission_timeout(0),
        Err(ConfigError::InvalidTimeout)
    );
    assert_eq!(
        config.set_transmission_timeout(15000),
        Err(ConfigError::InvalidTimeout)
    );
    ...
    assert_eq!(
        config.set_max_retry_attempts(0),
        Err(ConfigError::InvalidRetryCount)
    );
    assert_eq!(
        config.set_max_retry_attempts(15),
        Err(ConfigError::InvalidRetryCount)
    );
}
...
fn test_usb_control_commands() {
    ...
    assert_eq!(
        UsbControlCommand::from_u8(0x01),
        Some(UsbControlCommand::GetConfig)
    );
    assert_eq!(
        UsbControlCommand::from_u8(0x02),
        Some(UsbControlCommand::SetConfig)
    );
    assert_eq!(
        UsbControlCommand::from_u8(0x03),
        Some(UsbControlCommand::SetLogLevel)
    );
    assert_eq!(
        UsbControlCommand::from_u8(0x04),
        Some(UsbControlCommand::EnableCategory)
    );
    assert_eq!(
        UsbControlCommand::from_u8(0x05),
        Some(UsbControlCommand::DisableCategory)
    );
    assert_eq!(
        UsbControlCommand::from_u8(0x06),
        Some(UsbControlCommand::ResetConfig)
    );
    assert_eq!(
        UsbControlCommand::from_u8(0x07),
        Some(UsbControlCommand::GetStats)
    );
...
fn test_config_error_messages() {
    ...
    assert_eq!(
        ConfigError::CategoryDisabledAtCompileTime.as_str(),
        "Category disabled at compile time"
    );
}
...
fn test_configuration_combinations() {
    ...
    assert_eq!(
        selective_config.should_log(LogLevel::Info, LogCategory::Battery),
    ...
    assert_eq!(
        selective_config.should_log(LogLevel::Info, LogCategory::System),

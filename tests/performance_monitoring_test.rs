...
use ass_easy_loop::logging::{
    CpuUsageStats, LogLevel, LogMessage, LogQueue, MemoryUsageStats, MessagePerformanceStats,
    PerformanceMonitor, PerformanceStats, TimingImpactStats,
};
#[cfg(not(test))]
use panic_halt as _;
...
fn test_cpu_usage_calculation() {
    ...
    assert_eq!(
        PerformanceMonitor::calculate_cpu_usage(100, USB_POLL_INTERVAL_US),
        1
    ); // 1% for 100us execution
    assert_eq!(
        PerformanceMonitor::calculate_cpu_usage(500, USB_HID_INTERVAL_US),
        2
    ); // 2.5% rounded down for 500us execution
    ...
    assert!(
        PerformanceMonitor::calculate_cpu_usage(max_execution_time_us, USB_POLL_INTERVAL_US)
            <= max_allowed_cpu as u8
    );
}
...
fn test_queue_memory_calculation() {
    // For statically allocated queues, memory usage is always the full size
    // regardless of current item count

    // Test empty queue - should be full size
    assert_eq!(
        PerformanceMonitor::calculate_queue_memory_usage::<32>(0),
        core::mem::size_of::<LogQueue<32>>()
    );

    // Test partially filled queue - should still be full size
    let half_full = PerformanceMonitor::calculate_queue_memory_usage::<32>(16);
    let expected_half = core::mem::size_of::<LogQueue<32>>();
    assert_eq!(half_full, expected_half);

    // Test full queue - should be full size
    ...
    // Test different queue sizes - larger queue should use more memory
    assert!(
        PerformanceMonitor::calculate_queue_memory_usage::<64>(32)
...
fn test_batch_message_processing() {
    ...
    let large_batch: Vec<LogMessage> = (0..40)
        .map(|_| LogMessage::new(0, LogLevel::Debug, "TEST", "msg"))
        .collect();
...
fn test_performance_thresholds() {
    ...
    assert!(max_cpu_percent > 0.0);
    assert!(max_cpu_percent <= 50.0); // Should be reasonable limit
    ...
    perf_stats.update_memory_usage(warning_memory_bytes + 2048, 0);

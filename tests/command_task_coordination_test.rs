...
use ass_easy_loop::command::{
    AuthenticationValidator, CommandQueue, CommandReport, ParseResult, ResponseQueue,
};
...
fn test_command_queue_fifo_order() {
    ...
    assert_eq!(
        dequeued1.command.command_type,
    ...
    assert_eq!(
        dequeued2.command.command_type,
        TestCommand::ExecuteTest as u8
    );

    let dequeued3 = queue.dequeue().unwrap();
    assert_eq!(dequeued3.command.command_id, 3);
    assert_eq!(
        dequeued3.command.command_type,
...
fn test_command_authentication() {
    ...
    assert!(!AuthenticationValidator::validate_command(
        &corrupted_command
    ));
...
fn test_task_priority_simulation() {
    ...
    for i in 0..5 {
        let cmd = CommandReport::new(TestCommand::PerformanceMetrics as u8, i, &[i as u8]).unwrap();

...
mod tests {
    use ass_easy_loop::test_processor::{
        MemoryUsageMonitor, StressPattern, StressTestParameters, StressTestStatistics,
        TestParameterError,
    };
    ...
    fn test_stress_test_parameter_serialization() {
        ...
        assert_eq!(parsed_params.duration_ms, params.duration_ms);
        assert_eq!(parsed_params.load_level, params.load_level);
        assert_eq!(
            parsed_params.memory_stress_enabled,
            params.memory_stress_enabled
        );
        assert_eq!(parsed_params.cpu_stress_enabled, params.cpu_stress_enabled);
        assert_eq!(parsed_params.io_stress_enabled, params.io_stress_enabled);
        assert_eq!(
            parsed_params.concurrent_operations,
            params.concurrent_operations
        );
        assert_eq!(parsed_params.stress_pattern, params.stress_pattern);
        assert_eq!(
            parsed_params.performance_threshold_percent,
            params.performance_threshold_percent
        );
    }
    ...
    fn test_stress_test_statistics() {
        ...
        assert!((success_rate as f32 - 92.59).abs() < 0.1); // ~92.59% success rate (150/162)

        // Test performance criteria checking
        assert!(stats.meets_performance_criteria(70, 15)); // Should meet lenient criteria
        assert!(!stats.meets_performance_criteria(90, 5)); // Should not meet strict criteria
    ...
    fn test_stress_patterns() {
        ...
        assert_eq!(StressPattern::from_u8(0x00), Some(StressPattern::Constant));
        assert_eq!(StressPattern::from_u8(0x01), Some(StressPattern::Ramp));
        assert_eq!(StressPattern::from_u8(0x02), Some(StressPattern::Burst));
        assert_eq!(StressPattern::from_u8(0x03), Some(StressPattern::Random));
        assert_eq!(StressPattern::from_u8(0xFF), None); // Invalid pattern

        // Test pattern serialization
        assert_eq!(StressPattern::Constant as u8, 0x00);
        assert_eq!(StressPattern::Ramp as u8, 0x01);
        assert_eq!(StressPattern::Burst as u8, 0x02);
        assert_eq!(StressPattern::Random as u8, 0x03);
    }
    ...
    fn test_memory_monitor_edge_cases() {
        ...
        let stats = monitor.get_statistics();
        assert!(stats.memory_fragmentation_percent <= 50); // Should be capped at 50%
    }
    ...
    fn test_stress_statistics_edge_cases() {
        ...
        assert_eq!(zero_ops_stats.success_rate_percent(), 100.0);

        // Should meet any reasonable performance criteria
        assert!(zero_ops_stats.meets_performance_criteria(90, 0));
        ...
        assert_eq!(all_failed_stats.success_rate_percent(), 0.0);

        // Should not meet strict performance criteria
        assert!(!all_failed_stats.meets_performance_criteria(50, 5));

        // Should not meet any reasonable performance criteria due to low stability score
        assert!(!all_failed_stats.meets_performance_criteria(10, 200));
    }
}

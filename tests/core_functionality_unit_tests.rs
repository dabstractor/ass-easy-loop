...
mod battery_state_machine_tests {
    ...
    fn test_battery_state_boundary_conditions() {
        ...
        assert_eq!(
            BatteryState::from_adc_reading(1425),
            BatteryState::Low,
            "ADC 1425 should be Low state (≤ threshold)"
        );
        assert_eq!(
            BatteryState::from_adc_reading(1426),
            BatteryState::Normal,
            "ADC 1426 should be Normal state (> low threshold)"
        );
        assert_eq!(
            BatteryState::from_adc_reading(1674),
            BatteryState::Normal,
            "ADC 1674 should be Normal state (< charging threshold)"
        );
        assert_eq!(
            BatteryState::from_adc_reading(1675),
            BatteryState::Charging,
        ...
        assert_eq!(
            BatteryState::from_adc_reading(u16::MAX),
            BatteryState::Charging
        );
    }
    ...
    fn test_battery_state_thresholds() {
        ...
        assert_eq!(
            normal_min, 1425,
            "Normal state minimum threshold should be 1425"
        );
        assert_eq!(
            normal_max, 1675,
            "Normal state maximum threshold should be 1675"
        );
        ...
        assert_eq!(
            charging_min, 1675,
            "Charging state minimum threshold should be 1675"
        );
        assert_eq!(
            charging_max,
            u16::MAX,
            "Charging state maximum threshold should be u16::MAX"
        );
    }
    ...
    fn test_battery_state_transitions() {
        ...
        // Transition to Low
        assert_eq!(
            normal_state.should_transition_to(1400),
            Some(BatteryState::Low)
        );
        assert_eq!(
            normal_state.should_transition_to(1425),
            Some(BatteryState::Low)
        );

        // Transition to Charging
        assert_eq!(
            normal_state.should_transition_to(1675),
            Some(BatteryState::Charging)
        );
        assert_eq!(
            normal_state.should_transition_to(1800),
            Some(BatteryState::Charging)
        );
        ...
        assert_eq!(
            low_state.should_transition_to(1426),
            Some(BatteryState::Normal)
        );
        assert_eq!(
            low_state.should_transition_to(1500),
            Some(BatteryState::Normal)
        );

        // Transition to Charging (direct jump)
        assert_eq!(
            low_state.should_transition_to(1675),
            Some(BatteryState::Charging)
        );
        assert_eq!(
            low_state.should_transition_to(1800),
            Some(BatteryState::Charging)
        );
        ...
        // Transition to Normal
        assert_eq!(
            charging_state.should_transition_to(1674),
            Some(BatteryState::Normal)
        );
        assert_eq!(
            charging_state.should_transition_to(1500),
            Some(BatteryState::Normal)
        );

        // Transition to Low (direct jump)
        assert_eq!(
            charging_state.should_transition_to(1425),
            Some(BatteryState::Low)
        );
        assert_eq!(
            charging_state.should_transition_to(1000),
            Some(BatteryState::Low)
        );
...
mod adc_conversion_tests {
    ...
    fn test_adc_to_battery_voltage_conversion() {
        ...
        assert!(
            low_threshold_voltage >= 3350 && low_threshold_voltage <= 3450,
            "Low threshold (ADC 1425) should be ~3400mV, got {}mV",
            low_threshold_voltage
        );

        let charging_threshold_voltage = BatteryMonitor::adc_to_battery_voltage(1675);
        assert!(
            charging_threshold_voltage >= 3950 && charging_threshold_voltage <= 4050,
            "Charging threshold (ADC 1675) should be ~4000mV, got {}mV",
            charging_threshold_voltage
        );

        // Test maximum ADC value
        let max_voltage = BatteryMonitor::adc_to_battery_voltage(4095);
        assert!(
            max_voltage >= 9700 && max_voltage <= 9800,
            "Max ADC (4095) should be ~9770mV, got {}mV",
            max_voltage
        );
        ...
        assert!(
            mid_voltage >= 4800 && mid_voltage <= 5000,
            "Mid ADC (2048) should be ~4900mV, got {}mV",
            mid_voltage
        );
    }
    ...
    fn test_battery_voltage_to_adc_conversion() {
        ...
        let low_threshold_adc = BatteryMonitor::battery_voltage_to_adc(3400);
        assert!(
            low_threshold_adc >= 1400 && low_threshold_adc <= 1450,
            "3400mV should convert to ~1425 ADC, got {}",
            low_threshold_adc
        );

        let charging_threshold_adc = BatteryMonitor::battery_voltage_to_adc(4000);
        assert!(
            charging_threshold_adc >= 1650 && charging_threshold_adc <= 1700,
            "4000mV should convert to ~1675 ADC, got {}",
            charging_threshold_adc
        );
        ...
        assert_eq!(
            max_adc, 4095,
            "High voltage should clamp to maximum ADC value"
        );
        ...
        assert!(
            typical_normal_adc > 1425 && typical_normal_adc < 1675,
            "3700mV should be in Normal range"
        );
        assert!(
            typical_charging_adc >= 1675,
            "4200mV should be in Charging range"
        );
    }
    ...
    fn test_voltage_divider_constants() {
        ...
        assert!(
            (EXPECTED_RATIO - 0.337).abs() < 0.001,
            "Voltage divider ratio should be ~0.337, calculated: {:.3}",
            EXPECTED_RATIO
        );
        ...
        assert!(
            (EXPECTED_CONVERSION_FACTOR - IMPLEMENTATION_FACTOR).abs() < 0.01,
...
mod timing_calculation_tests {
    ...
    fn test_pemf_pulse_timing_constants() {
        ...
        assert!(
            (frequency_hz - 2.0).abs() < 0.001,
            "Frequency should be 2Hz, calculated: {:.3}Hz",
            frequency_hz
        );

        // Verify pulse width is exactly 2ms as required
        assert_eq!(
            PULSE_HIGH_DURATION_MS, 2,
            "Pulse HIGH duration must be exactly 2ms"
        );

        // Verify low phase duration
        assert_eq!(
            PULSE_LOW_DURATION_MS, 498,
            "Pulse LOW duration must be exactly 498ms"
        );
    }
    ...
    fn test_timing_accuracy_tolerance() {
        ...
        assert!(
            high_min <= 2.0 && 2.0 <= high_max,
            "HIGH pulse timing within ±1% tolerance: {:.3}ms - {:.3}ms",
            high_min,
            high_max
        );
        assert!(
            low_min <= 498.0 && 498.0 <= low_max,
            "LOW pulse timing within ±1% tolerance: {:.3}ms - {:.3}ms",
            low_min,
            low_max
        );
        ...
        assert!(
            period_min <= total_period as f32 && total_period as f32 <= period_max,
            "Total period within ±1% tolerance: {:.3}ms - {:.3}ms",
            period_min,
            period_max
        );
    }
    ...
    fn test_pulse_state_tracking() {
        ...
        assert!(
            pulse_active,
            "pulse_active should be true during HIGH phase"
        );
        ...
        assert!(
            !pulse_active,
            "pulse_active should be false during LOW phase"
        );
        ...
        for cycle in 0..10 {
            ...
            assert!(
                pulse_active,
                "pulse_active should be true during HIGH phase of cycle {}",
                cycle
            );
            ...
            assert!(
                !pulse_active,
                "pulse_active should be false during LOW phase of cycle {}",
                cycle
            );
        }
    }
    ...
    fn test_frequency_calculation() {
        ...
        assert!(
            (calculated_frequency - 2.0).abs() < 0.001,
            "Calculated frequency should be 2Hz, got {:.3}Hz",
        ...
        assert!(
            (duty_cycle - expected_duty_cycle).abs() < 0.01,
            "Duty cycle should be 0.4%, got {:.2}%",
            duty_cycle
        );
    }
    ...
    fn test_timing_precision_requirements() {
        ...
        assert!(
            high_max_deviation >= 0.02,
            "HIGH pulse precision tolerance should be at least 0.02ms, got {:.3}ms",
            high_max_deviation
        );
        assert!(
            low_max_deviation >= 4.98,
            "LOW pulse precision tolerance should be at least 4.98ms, got {:.3}ms",
            low_max_deviation
        );
        ...
        assert!(
            total_max_deviation >= 5.0,
            "Total period precision tolerance should be at least 5ms, got {:.3}ms",
            total_max_deviation
        );
    }
    ...
    fn test_gpio_control_timing() {
        ...
        assert!(
            effective_high_time > 1.9,
            "HIGH pulse should have sufficient time after GPIO overhead: {:.3}ms",
            effective_high_time
        );
        assert!(
            effective_low_time > 497.9,
            "LOW pulse should have sufficient time after GPIO overhead: {:.3}ms",
            effective_low_time
        );
    }
    ...
    fn test_pulse_generation_cycle_consistency() {
        ...
        assert!(
            (average_frequency - 2.0).abs() < 0.001,
...
mod core_integration_tests {
    ...
    fn test_battery_state_adc_integration() {
        ...
        let test_scenarios = vec![
            ...
            (3300, BatteryState::Low, "At low threshold"),
            (3500, BatteryState::Normal, "Normal operating voltage"),
            (3700, BatteryState::Normal, "Good battery level"),
            (3900, BatteryState::Normal, "Near charging threshold"),
            (4000, BatteryState::Charging, "Charging detected"),
    ...
    fn test_timing_battery_integration() {
        ...
        assert!(
            MAX_BATTERY_PROCESSING_TIME_MS < BATTERY_SAMPLE_INTERVAL_MS,
    ...
    fn test_comprehensive_core_validation() {
        ...
        for adc_value in adc_test_values {
            ...
            match current_state {
                BatteryState::Low => assert!(
                    voltage <= 3400,
                    "Low state voltage should be ≤3400mV, got {}mV",
                    voltage
                ),
                BatteryState::Normal => assert!(
                    voltage > 3000 && voltage < 3800,
                    "Normal state voltage should be 3000-3800mV, got {}mV",
                    voltage
                ),
                BatteryState::Charging => assert!(
                    voltage >= 3500,
                    "Charging state voltage should be ≥3500mV, got {}mV",
                    voltage
                ),
            }
        ...
        assert!(
            state_changes >= 2,
            "Should detect multiple state changes, got {}",
            state_changes
        );
        ...
        assert_eq!(
            battery_samples_per_pulse, 5,
            "Should have 5 battery samples per pulse cycle"
        );
    }
}

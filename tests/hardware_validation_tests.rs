...
impl HardwareTestSuite {
    ...
    pub fn check_device_connection(&mut self) -> bool {
        ...
        let lsusb_result = Command::new("lsusb").output();

        match lsusb_result {
            Ok(output) => {
                let output_str = String::from_utf8_lossy(&output.stdout);
                let device_found = output_str
                    .contains(&format!("{:04x}:{:04x}", TEST_DEVICE_VID, TEST_DEVICE_PID));
        ...
        match python_test {
            Ok(output) => {
                ...
                let result = if success {
                    ...
                    HardwareTestResult::new("Device Enumeration").failure(duration, &error_str)
                };
    ...
    fn test_log_message_reception(&mut self) -> bool {
        ...
        match hidlog_test {
            ...
            Err(e) => {
                ...
                let result = HardwareTestResult::new("Log Message Reception").failure(
                    duration,
                    &format!("scripts/utilities/hidlog.py execution failed: {}", e),
                );
                self.results.push(result);
                false
            }
...
"#,
        ...
        let result = if success_rate >= 80.0 {
            println!(
                ...
                success_rate, successful_connections, test_cycles
            );
...
"#,
        ...
        match timing_test {
            Ok(output) => {
                ...
                let result = if success {
                    ...
                    HardwareTestResult::new("pEMF Timing Accuracy").failure(duration, &error_str)
                };
    ...
    fn validate_timing_constants(&self) -> bool {
        ...
        let frequency_error =
            ((calculated_frequency - PEMF_FREQUENCY_HZ) / PEMF_FREQUENCY_HZ * 100.0).abs();
        ...
        if timing_valid {
            println!(
                "  ✓ Timing constants within ±{}% tolerance",
                TIMING_TOLERANCE_PERCENT
            );
        } else {
            println!(
                "  ✗ Timing constants exceed ±{}% tolerance",
                TIMING_TOLERANCE_PERCENT
            );
        }
...
"#,
    ...
    fn validate_adc_conversion(&self) -> bool {
        ...
        for (adc_value, expected_mv) in test_cases {
            ...
            let error_percent =
                ((calculated_mv as f32 - expected_mv as f32) / expected_mv as f32 * 100.0).abs();

            println!(
                "    ADC {} -> {}mV (expected ~{}mV, error: {:.1}%)",
                adc_value, calculated_mv, expected_mv, error_percent
            );

            if error_percent > 10.0 {
                // Allow 10% tolerance for conversion
                all_valid = false;
            }
        }
        ...
        match integration_test {
            Ok(output) => {
                ...
                let result = if success {
                    ...
                    HardwareTestResult::new("System Integration").failure(duration, &error_str)
                };
...
"#, TEST_DEVICE_VID, TEST_DEVICE_PID))
        ...
        match performance_test {
            Ok(output) => {
                ...
                let result = if success {
                    ...
                    HardwareTestResult::new("Performance Under Load").failure(duration, &error_str)
                };
    ...
    fn print_test_summary(&self) {
        ...
        if failed_tests == 0 {
            ...
            println!(
                "⚠️  {} test(s) failed. Please review the errors above.",
                failed_tests
            );
...
fn test_hardware_device_connection() {
    ...
    if !connected {
        ...
        println!(
            "3. Verify device appears in: lsusb | grep {:04x}:{:04x}",
            TEST_DEVICE_VID, TEST_DEVICE_PID
        );
        return;
    }
    ...
    assert!(
        connected,
        "RP2040 device should be connected and accessible"
    );
}
...
fn test_hardware_usb_hid_communication() {
    ...
    assert!(
        success,
        "USB HID communication should work with real hardware"
    );
}
...
fn test_hardware_pemf_timing() {
    ...
    assert!(
        success,
        "pEMF timing should be accurate with USB logging active"
    );
}
...
fn test_hardware_battery_monitoring() {
    ...
    assert!(
        success,
        "Battery monitoring should work with actual ADC readings"
    );
}

...
// Test files use std by default
use std::string::ToString;

use ass_easy_loop::battery::{BatteryMonitor, BatteryState};
use ass_easy_loop::config;
use ass_easy_loop::logging::{init_global_logging, LogLevel, LogMessage, LogQueue};
...
fn test_battery_state_change_logging() {
    ...
    for (from_state, to_state, expected_message_part) in test_cases {
        ...
        println!(
            "State transition: {:?} -> {:?}, ADC: {}, Voltage: {}mV",
...
fn test_battery_voltage_calculation_accuracy() {
    ...
    let test_cases = vec![
        ...
        (1425, 3400), // Low threshold ~3.4V (1425 * 2386 / 1000 = 3400)
        (1675, 3996), // Charging threshold ~4.0V (1675 * 2386 / 1000 = 3996)
        (2048, 4886), // Mid-range value (2048 * 2386 / 1000 = 4886)
        (4095, 9770), // Max ADC value (4095 * 2386 / 1000 = 9770)
    ];
...
fn test_periodic_logging_interval_configuration() {
    ...
    assert!(
        interval >= 10,
        "Periodic logging interval too frequent: {}",
        interval
    );
    assert!(
        interval <= 600,
        "Periodic logging interval too infrequent: {}",
        interval
    );
    ...
    assert!(
        ...
        "Periodic logging less than 1 second"
    );
    assert!(
        time_interval_seconds <= 60.0,
        "Periodic logging more than 1 minute"
    );

    println!(
        "Periodic logging interval: {} samples = {:.1} seconds",
...
fn test_logging_message_format_for_battery_data() {
    ...
    let test_cases = vec![
        ...
        (
            1500,
            BatteryState::Normal,
            "should format normal battery message",
        ),
        (
            1700,
            BatteryState::Charging,
            "should format charging battery message",
        ),
    ];
...
fn test_battery_logging_configuration_validation() {
    ...
    // Verify battery logging configuration (may be enabled or disabled based on features)
    // When battery-logs feature is enabled, it should be true
    // When battery-logs feature is disabled, it should be false
    #[cfg(feature = "battery-logs")]
    assert!(
        config::logging::ENABLE_BATTERY_LOGS,
        "Battery logging should be enabled when battery-logs feature is active"
    );

    #[cfg(not(feature = "battery-logs"))]
    assert!(
        !config::logging::ENABLE_BATTERY_LOGS,
        "Battery logging should be disabled when battery-logs feature is not active"
    );
    ...
    assert_eq!(
        ...
        "Battery monitoring should run at 100ms intervals"
    );
    ...
    assert_eq!(
        calculated_interval, battery_interval_ms,

...
fn test_usb_logging_performance_impact_on_pemf_timing() {
    // Test that USB logging operations don't significantly impact pEMF timing requirements
    // Requirements: 2.3 (pulse timing SHALL remain accurate within ±1% tolerance under any system load)
    // Note: This test validates that USB logging overhead is minimal, not precise hardware timing
    
    let mut queue: LogQueue<32> = LogQueue::new();
    
    // Target timing: 2ms HIGH, 498ms LOW (2Hz square wave)
    let target_high_duration_us = 2000; // 2ms
    let target_low_duration_us = 498000; // 498ms
    
    // Test 1: Verify USB logging operations are fast relative to pEMF timing requirements
    let mut logging_times = Vec::new();
    
    for i in 0..100 {
        let start = Instant::now();
        
        // Perform USB logging operations
        let message = LogMessage::new(i, LogLevel::Info, "PEMF", "Pulse cycle");
        let _ = queue.enqueue(message);
        
        logging_times.push(start.elapsed());
    }
    
    // Calculate average USB logging time
    let avg_logging_time = logging_times.iter().sum::<Duration>() / logging_times.len() as u32;
    let avg_logging_us = avg_logging_time.as_micros() as f64;
    
    // Verify USB logging overhead is much smaller than pEMF timing requirements
    // HIGH pulse is 2ms (2000μs) - logging should be <5% of this (100μs) in test environment
    // (In embedded systems with hardware timers, this would be much smaller)
    let high_tolerance_us = target_high_duration_us as f64 * 0.05; // 5% of HIGH pulse for test environment
    let low_tolerance_us = target_low_duration_us as f64 * 0.05;   // 5% of LOW pulse for test environment
    
    assert!(
        avg_logging_us < high_tolerance_us,
        "USB logging time {:.2}μs exceeds 5% of HIGH pulse time ({:.2}μs)", 
        avg_logging_us, high_tolerance_us
    );
    
    assert!(
        avg_logging_us < low_tolerance_us,
        "USB logging time {:.2}μs exceeds 5% of LOW pulse time ({:.2}μs)", 
        avg_logging_us, low_tolerance_us
    );
    
    // Test 2: Verify queue operations don't cause significant delays
    let queue_full_start = Instant::now();
    
    // Fill the queue to test performance under load
    for i in 0..32 {
        let message = LogMessage::new(i + 100, LogLevel::Info, "PEMF", "Queue stress test");
        let _ = queue.enqueue(message);
    }
    
    let queue_full_time = queue_full_start.elapsed();
    let queue_full_us = queue_full_time.as_micros() as f64;
    
    // Even filling the entire queue should be fast relative to pEMF timing
    assert!(
        queue_full_us < high_tolerance_us,
        "Queue fill time {:.2}μs exceeds 5% of HIGH pulse time ({:.2}μs)", 
        queue_full_us, high_tolerance_us
    );
    
    // Test 3: Verify queue overflow handling doesn't cause timing issues
    let overflow_start = Instant::now();
    
    // Try to add more messages than queue capacity (should handle gracefully)
    for i in 0..10 {
        let message = LogMessage::new(i + 200, LogLevel::Info, "PEMF", "Overflow test");
        let _ = queue.enqueue(message); // Should handle overflow gracefully
    }
    
    let overflow_time = overflow_start.elapsed();
    let overflow_us = overflow_time.as_micros() as f64;
    
    // Overflow handling should also be fast
    assert!(
        overflow_us < high_tolerance_us,
        "Queue overflow handling time {:.2}μs exceeds 5% of HIGH pulse time ({:.2}μs)", 
        overflow_us, high_tolerance_us
    );
}
...
fn test_memory_usage_impact_with_usb_logging() {
    ...
    for cycle in 0..10 {
        // Fill queue
        for i in 0..20 {
            // Overfill to test overflow handling
...
fn test_queue_overflow_recovery_during_usb_errors() {
    ...
    for i in 0..16 {
        // More messages than queue capacity

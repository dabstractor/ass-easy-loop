...
mod tests {
    ...
    fn test_battery_state_transitions() {
        ...
        assert_eq!(battery_state_from_adc(1296), 0); // Low (at threshold)
        assert_eq!(battery_state_from_adc(1297), 1); // Normal
        assert_eq!(battery_state_from_adc(1400), 1); // Normal
        assert_eq!(battery_state_from_adc(1507), 1); // Normal
        assert_eq!(battery_state_from_adc(1508), 2); // Charging (at threshold)
        assert_eq!(battery_state_from_adc(2000), 2); // Charging

        // Test valid transitions
        assert!(is_valid_transition(0, 1, 1400)); // Low -> Normal with appropriate ADC
        assert!(is_valid_transition(1, 0, 1200)); // Normal -> Low with appropriate ADC
        assert!(is_valid_transition(1, 2, 1600)); // Normal -> Charging with appropriate ADC
        assert!(is_valid_transition(2, 1, 1400)); // Charging -> Normal with appropriate ADC
        assert!(is_valid_transition(0, 2, 1600)); // Low -> Charging with appropriate ADC
        assert!(is_valid_transition(2, 0, 1200)); // Charging -> Low with appropriate ADC

        // Test invalid transitions
        assert!(!is_valid_transition(0, 1, 1200)); // Low -> Normal with ADC too low
        assert!(!is_valid_transition(1, 2, 1400)); // Normal -> Charging with ADC too low
        assert!(!is_valid_transition(2, 0, 1600)); // Charging -> Low with ADC too high
    }

    #[test]
    fn test_adc_to_voltage_conversion() {
        // Test voltage conversion calculations
        // ADC 1296 should be approximately 3100mV (3.1V) per spec
        let voltage_1296 = adc_to_battery_voltage(1296);
        assert!(voltage_1296 >= 3000 && voltage_1296 <= 3200);

        // ADC 1508 should be approximately 3600mV (3.6V) per spec
        let voltage_1508 = adc_to_battery_voltage(1508);
        assert!(voltage_1508 >= 3500 && voltage_1508 <= 3700);

        // Test reverse conversion
        let adc_from_3100mv = battery_voltage_to_adc(3100);
        assert!(adc_from_3100mv >= 1290 && adc_from_3100mv <= 1310);

        let adc_from_3600mv = battery_voltage_to_adc(3600);
        assert!(adc_from_3600mv >= 1500 && adc_from_3600mv <= 1520);
    ...
    fn test_battery_adc_test_result_evaluation() {
        ...
        let mut result = create_test_result(3600, 2.0, 1508); // 3.6V reference, 2% tolerance, 1508 ADC expected

        // Add good samples
        add_test_sample(&mut result, 1490); // Within tolerance
        add_test_sample(&mut result, 1508); // Perfect
        add_test_sample(&mut result, 1526); // Within tolerance

        // Add valid state transition
        add_test_transition(&mut result, 1, 2, 1508); // Normal -> Charging with valid ADC
    ...
    fn is_valid_transition(from_state: u8, to_state: u8, adc_value: u16) -> bool {
        match (from_state, to_state) {
            (0, 1) => adc_value > 1296,  // Low -> Normal
            (1, 0) => adc_value <= 1296, // Normal -> Low
            (1, 2) => adc_value >= 1508, // Normal -> Charging
            (2, 1) => adc_value < 1508,  // Charging -> Normal
            (0, 2) => adc_value >= 1508, // Low -> Charging
            (2, 0) => adc_value <= 1296, // Charging -> Low
    ...
    fn calculate_voltage_accuracy(
        measurements: &mut AdcMeasurements,
        reference_voltage_mv: u32,
    ) -> f32 {
    ...
    fn add_test_transition(result: &mut TestResult, from_state: u8, to_state: u8, adc_value: u16) {
        ...
        result
            .transitions
            .push((from_state, to_state, adc_value, valid));
    }
    ...
    fn complete_test(result: &mut TestResult) {
        ...
        let voltage_accuracy =
            100.0 - (voltage_error as f32 / result.reference_voltage_mv as f32 * 100.0);
        ...
        let invalid_reading_rate =
            (result.invalid_readings_count as f32 / result.samples.len() as f32) * 100.0;

...
impl TaskShutdownSequence {
    ...
    pub fn update_shutdown_progress(
        &mut self,
        current_time_ms: u32,
    ) -> Result<bool, BootloaderError> {
        let start_time = self
            .shutdown_start_time
            .ok_or(BootloaderError::TaskShutdownFailed)?;

        // Check for overall timeout
        if current_time_ms.saturating_sub(start_time) > self.shutdown_timeout_ms {
            log_error!(
                "Task shutdown sequence timed out after {}ms",
        ...
        match (
            self.high_priority_status,
            self.medium_priority_status,
    ...
    pub fn get_shutdown_status(
        &self,
    ) -> (TaskShutdownStatus, TaskShutdownStatus, TaskShutdownStatus) {
        (
            self.high_priority_status,
            self.medium_priority_status,
            self.low_priority_status,
...
impl HardwareSafetyManager {
    ...
    pub fn validate_hardware_state(
        &self,
        ...
        log_info!(
            "  - USB: {} (safe)",
            if hardware_state.usb_transmitting {
                "TRANSMITTING"
            } else {
                "IDLE"
            }
    ...
    pub fn update_entry_progress(
        ...
        match self.entry_state {
            ...
            BootloaderEntryState::ShuttingDownTasks => {
                ...
                match self
                    .shutdown_sequence
                    .update_shutdown_progress(current_time_ms)
                {
                    Ok(true) => {
                ...
                match self.hardware_safety.validate_hardware_state(hardware_state) {
                    ...
                    Err(_) => {
                        ...
                        match self.hardware_safety.force_safe_state() {
                            Ok(()) => {
                                log_info!(
                                    "Hardware forced into safe state, ready for bootloader entry"
                                );
                                self.entry_state = BootloaderEntryState::ReadyForBootloader;
                            }
                            Err(e) => {

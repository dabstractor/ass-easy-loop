...
use hal::{
    adc::{Adc, AdcPin},
    gpio::{bank0::Gpio26, FunctionSio, Pin, PullNone, SioInput},
};
use rp2040_hal as hal;

use crate::types::{AdcValue, VoltageMillivolts};

#[derive(Debug, Clone, Copy, PartialEq)]
pub enum BatteryState {
    Low,      // ADC ≤ 1425 (per requirement 3.2)
    Normal,   // 1425 < ADC < 1675 (per requirement 3.3)
    Charging, // ADC ≥ 1675 (per requirement 3.4)
}

impl BatteryState {
    /// Determine battery state from ADC reading with threshold comparisons
    pub fn from_adc_reading(adc_value: AdcValue) -> Self {
        if adc_value <= 1425 {
            BatteryState::Low
    ...
    pub fn should_transition_to(&self, adc_value: AdcValue) -> Option<BatteryState> {
    ...
    pub fn is_valid_transition(&self, to_state: BatteryState, adc_value: AdcValue) -> bool {
...
pub fn validate_battery_state_logic() -> bool {
    ...
    if BatteryState::from_adc_reading(0) != BatteryState::Low {
        return false;
    }
    if BatteryState::from_adc_reading(1000) != BatteryState::Low {
        return false;
    }
    if BatteryState::from_adc_reading(1425) != BatteryState::Low {
        return false;
    }

    // Test Normal battery state (1425 < ADC < 1675)
    if BatteryState::from_adc_reading(1426) != BatteryState::Normal {
        return false;
    }
    if BatteryState::from_adc_reading(1500) != BatteryState::Normal {
        return false;
    }
    if BatteryState::from_adc_reading(1674) != BatteryState::Normal {
        return false;
    }

    // Test Charging battery state (ADC ≥ 1675)
    if BatteryState::from_adc_reading(1675) != BatteryState::Charging {
        return false;
    }
    if BatteryState::from_adc_reading(2000) != BatteryState::Charging {
        return false;
    }
    if BatteryState::from_adc_reading(4095) != BatteryState::Charging {
        return false;
    }
    ...
    match current_state.should_transition_to(1200) {
        Some(BatteryState::Low) => {}
        _ => return false,
    }

    // Test transition from Normal to Charging
    match current_state.should_transition_to(1600) {
...
impl BatteryMonitor {
    ...
    pub fn battery_voltage_to_adc(battery_voltage_mv: VoltageMillivolts) -> AdcValue {
    ...
    pub fn get_battery_voltage_mv(
        adc: &mut Adc,
        adc_pin: &mut AdcPin<Pin<Gpio26, FunctionSio<SioInput>, PullNone>>,
    ) -> Result<VoltageMillivolts, AdcError> {
        match Self::read_adc_value(adc, adc_pin) {
...
pub fn validate_adc_conversion_logic() -> bool {
    ...
    // ADC 1296 should be approximately 3100mV (3.1V)
    let voltage_1296 = BatteryMonitor::adc_to_battery_voltage(1296);
    if !(3000..=3200).contains(&voltage_1296) {
        return false;
    }

    // ADC 1508 should be approximately 3600mV (3.6V)
    let voltage_1508 = BatteryMonitor::adc_to_battery_voltage(1508);
    if !(3500..=3700).contains(&voltage_1508) {
        return false;
    }

    // Test reverse conversion
    let adc_from_3100mv = BatteryMonitor::battery_voltage_to_adc(3100);
    if !(1400..=1450).contains(&adc_from_3100mv) {
        return false;
    }

    let adc_from_3600mv = BatteryMonitor::battery_voltage_to_adc(3600);
    if !(1650..=1700).contains(&adc_from_3600mv) {
        return false;
    }
    ...
    if !(9700..=9800).contains(&voltage_4095) {
        return false;
    } // Should be ~9.77V

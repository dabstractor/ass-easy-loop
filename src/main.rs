...
use ass_easy_loop::{
    config, init_bootloader_manager, mark_task_shutdown_complete, should_task_shutdown,
    AuthenticationValidator, BatteryState, BootloaderEntryManager, BootloaderEntryState,
    CommandQueue, CommandReport, ErrorRecovery, HardwareState, LogLevel, LogQueue, LogReport,
    ParseResult, ResourceLeakDetector, ResourceValidator, ResponseQueue, StateQueryType,
    SystemError, SystemResult, SystemStateHandler, TaskPriority, TestCommandProcessor, TestStatus,
};

#[cfg(all(not(test), not(any(feature = "std-testing", target_arch = "x86_64"))))]
use ass_easy_loop::LogMessage;

// Import logging macros from crate root
use ass_easy_loop::{
    init_global_logging, log_debug, log_error, log_info, log_system_debug, log_system_error,
    log_system_info, log_warn, BatteryMonitor, CommandParser,
};
...
static mut GLOBAL_PERFORMANCE_STATS: ass_easy_loop::logging::PerformanceStats =
    ass_easy_loop::logging::PerformanceStats::new();
...
fn panic(info: &core::panic::PanicInfo) -> ! {
    ...
    unsafe {
        if let (queue, Some(get_timestamp)) = (&raw mut GLOBAL_LOG_QUEUE, TIMESTAMP_FUNCTION) {
            ...
            let panic_log =
                LogMessage::new(timestamp, LogLevel::Error, "PANIC", panic_msg.as_str());
            ...
            let state_log = ass_easy_loop::logging::LogMessage::new(
                timestamp + 2,
                LogLevel::Error,
                "PANIC",
                state_msg.as_str(),
            ...
            let diag_log = ass_easy_loop::logging::LogMessage::new(
                timestamp + 3,
                LogLevel::Error,
                "PANIC",
                diag_msg.as_str(),
...
mod app {
    ...
    const _: () = assert!(
        PEMF_PULSE_PRIORITY > BATTERY_MONITOR_PRIORITY,
        "pEMF pulse must have higher priority than battery monitoring"
    );
    const _: () = assert!(
        PEMF_PULSE_PRIORITY > LED_CONTROL_PRIORITY,
        "pEMF pulse must have higher priority than LED control"
    );
    const _: () = assert!(
        BATTERY_MONITOR_PRIORITY > LED_CONTROL_PRIORITY,
        "Battery monitoring must have higher priority than LED control"
    );
    const _: () = assert!(
        PEMF_PULSE_PRIORITY > USB_HID_PRIORITY,
        "pEMF pulse must have higher priority than USB HID"
    );
    const _: () = assert!(
        PEMF_PULSE_PRIORITY > USB_POLL_PRIORITY,
        "pEMF pulse must have higher priority than USB polling"
    );
    ...
    fn init(mut ctx: init::Context) -> (Shared, Local) {
        ...
        let usb_dev = UsbDeviceBuilder::new(
            usb_bus_ref,
            UsbVidPid(config::usb::VENDOR_ID, config::usb::PRODUCT_ID),
        )
        .device_release(config::usb::DEVICE_RELEASE)
        ...
        unsafe {
            ...
            ass_easy_loop::init_global_config(&raw mut GLOBAL_LOG_CONFIG);
            ass_easy_loop::init_global_performance_monitoring(&raw mut GLOBAL_PERFORMANCE_STATS);
        ...
        log_info!(
            "USB HID report size: {} bytes",
            config::usb::HID_REPORT_SIZE
        );
        ...
        log_info!("- Log queue size: {} messages", unsafe {
            (*core::ptr::addr_of!(GLOBAL_LOG_QUEUE)).capacity()
        });
        ...
        #[cfg(feature = "test-commands")]
        {
            ...
            log_info!(
                "- Command queue capacity: {} commands",
        ...
        #[cfg(feature = "test-commands")]
        {
            ...
            log_info!(
                "- Supported tests: pEMF timing, battery ADC, LED, stress, USB communication"
            );
        }
    ...
    async fn pemf_pulse_task(ctx: pemf_pulse_task::Context) {
        ...
        log_info!(
            ...
            STATISTICS_LOG_INTERVAL_CYCLES,
            TIMING_VALIDATION_INTERVAL_CYCLES
        );
        ...
        loop {
            ...
            *pulse_active = true;
            ...
            let high_result = mosfet_pin
                .set_high()
                .map_err(|_| SystemError::GpioOperationFailed);
            ...
            *pulse_active = false;
            ...
            if cycle_count % STATISTICS_LOG_INTERVAL_CYCLES == 0 {
                ...
                log_info!(
                    "Average timing - HIGH: {}ms, LOW: {}ms, Total: {}ms",
                    avg_high_time_ms,
                    avg_low_time_ms,
                    avg_total_cycle_ms
                );
            ...
            if cycle_count % 5 == 0 {
                // Send measurement every 5 cycles to avoid overwhelming the test processor
    ...
    async fn battery_monitor_task(mut ctx: battery_monitor_task::Context) {
        ...
        const PERIODIC_LOG_INTERVAL_SAMPLES: u32 =
            config::logging::BATTERY_PERIODIC_LOG_INTERVAL_SAMPLES;
        ...
        loop {
            ...
            match adc_result {
                Ok(adc_value) => {
                    ...
                    let battery_voltage_mv = BatteryMonitor::adc_to_battery_voltage(adc_value);
                ...
                BatteryState::Low => {
                    ...
                    if !current_led_on {
                        ctx.shared.led.lock(|led| {
                            ...
                            // Requirements: 7.1 (graceful error handling for non-critical operations)
                            let result =
                                led.set_high().map_err(|_| SystemError::GpioOperationFailed);
                    ...
                    if current_led_on {
                        ctx.shared.led.lock(|led| {
                            ...
                            // Requirements: 7.1 (graceful error handling for non-critical operations)
                            let result =
                                led.set_low().map_err(|_| SystemError::GpioOperationFailed);
                            if let Err(error) = result {
                                let _ = ErrorRecovery::handle_error(
                                    error,
                                    "LED control LOW (flash OFF)",
                                );
                            }
                        });
                    ...
                    if !current_led_on || last_battery_state != battery_state {
                        ctx.shared.led.lock(|led| {
                            ...
                            if let Err(error) = result {
                                let _ = ErrorRecovery::handle_error(
                                    error,
                                    "LED control HIGH (charging state)",
                                );
                            }
                        });
                        current_led_on = true;
    ...
    async fn usb_poll_task(mut ctx: usb_poll_task::Context) {
        ...
        loop {
            ...
            let (poll_result, execution_time_us) =
                ass_easy_loop::logging::PerformanceMonitor::measure_task_execution(|| {
                                            ...
                                            if AuthenticationValidator::validate_command(&command) {
                                                ...
                                                if let Ok(()) = AuthenticationValidator::validate_format(&command) {
                                                    ...
                                                    if enqueue_success {
                                                        ...
                                                        let ack_msg = ass_easy_loop::logging::LogMessage::new(
                                                            timestamp,
                                                            LogLevel::Info,
                                                            "CMD",
                                                        ...
                                                        let error_msg = ass_easy_loop::logging::LogMessage::new(
                                                            timestamp,
                                                            LogLevel::Error,
                                                            "CMD",
                                                            "Command queue full"
            ...
            if cycle_count % CPU_MEASUREMENT_INTERVAL_CYCLES == 0 {
                ...
                let cpu_usage_percent =
                    ass_easy_loop::logging::PerformanceMonitor::calculate_cpu_usage(
                ...
                if cpu_usage_percent > crate::config::system::MAX_USB_CPU_USAGE_PERCENT as u8 {
                    log_warn!(
    ...
    async fn usb_command_handler_task(mut ctx: usb_command_handler_task::Context) {
        ...
        log_info!(
            "Command processing interval: {}ms",
            COMMAND_PROCESSING_INTERVAL_MS
        );
        ...
        loop {
            ...
            if current_time.saturating_sub(last_timeout_check) >= TIMEOUT_CHECK_INTERVAL_MS as u32 {
                let timed_out_count = ctx
                    .shared
                    .command_queue
                    .lock(|queue| queue.remove_timed_out_commands(current_time));
            ...
            let command_available = ctx.shared.command_queue.lock(|queue| !queue.is_empty());

            if command_available {
                // Process the next command (FIFO order)
                // Requirements: 2.4 (commands executed in FIFO order)
                let queued_command = ctx.shared.command_queue.lock(|queue| queue.dequeue());
                ...
                if let Some(queued_cmd) = queued_command {
                    ...
                    if queued_cmd.is_timed_out(timestamp) {
                        log_warn!(
                            "Command timed out before processing: type=0x{:02X}, id={}, seq={}",
                            queued_cmd.command.command_type,
                    ...
                    log_info!(
                        "Processing command: type=0x{:02X}, id={}, seq={}, remaining_timeout={}ms",
                        queued_cmd.command.command_type,
                        queued_cmd.command.command_id,
                        queued_cmd.sequence_number,
                        queued_cmd.remaining_timeout_ms(timestamp)
                    );

                    // Process the command based on its type
                    match queued_cmd.command.get_test_command() {
                        Some(ass_easy_loop::TestCommand::EnterBootloader) => {
                            ...
                            bootloader_entry_task::spawn(queued_cmd.command.command_id, timeout_ms)
                                .ok();
                            commands_processed += 1;
                        }
                        Some(ass_easy_loop::TestCommand::SystemStateQuery) => {
                            ...
                            if queued_cmd.command.payload.is_empty() {
                                ...
                                if let Ok(error_response) = CommandReport::error_response(
                                    queued_cmd.command.command_id,
                                    ass_easy_loop::ErrorCode::InvalidFormat,
                            ...
                            match StateQueryType::from_u8(query_type_byte) {
                                Some(query_type) => {
                                    ...
                                    let response_result =
                                        ctx.shared.system_state_handler.lock(|handler| {
                                    ...
                                    match response_result {
                                        Ok(state_data) => {
                                            ...
                                            if let Ok(response) = CommandReport::new(
                                                ass_easy_loop::TestResponse::StateData as u8,
                                                queued_cmd.command.command_id,
                                                state_data.as_slice(),
                                            ) {
                                                ctx.shared.response_queue.lock(|queue| {
                                        ...
                                        Err(error) => {
                                            ...
                                            if let Ok(error_response) =
                                                CommandReport::error_response(
                                                    queued_cmd.command.command_id,
                                                    ass_easy_loop::ErrorCode::SystemNotReady,
                                ...
                                None => {
                                    ...
                                    if let Ok(error_response) = CommandReport::error_response(
                                        queued_cmd.command.command_id,
                                        ass_easy_loop::ErrorCode::UnsupportedCommand,
                                        "Invalid query type",
                                    ) {
                                        ctx.shared.response_queue.lock(|queue| {
                                            queue.enqueue(
                                    ...
                                    commands_failed += 1;
                                    log_warn!(
                                        "Invalid system state query type: 0x{:02X}",
                                        query_type_byte
                                    );
                                }
                            }
                        }
                        Some(ass_easy_loop::TestCommand::ExecuteTest) => {
                            log_info!(
                                "Execute test command received - processing with test processor"
                            );
                            ...
                            match response_result {
                                Ok(response) => {
                                    ...
                                    commands_processed += 1;
                                    log_info!(
                                        "Test command processed successfully, response queued"
                                    );
                                }
                                Err(error_code) => {
                        ...
                        Some(ass_easy_loop::TestCommand::ConfigurationQuery) => {
                        ...
                        Some(ass_easy_loop::TestCommand::RunTestSuite) => {
                        ...
                        Some(ass_easy_loop::TestCommand::GetTestResults) => {
                        ...
                        Some(ass_easy_loop::TestCommand::ClearTestResults) => {
                        ...
                        None => {
                            ...
                            log_warn!(
                                "Unknown command type: 0x{:02X}, seq={}",
                                queued_cmd.command.command_type,
                                queued_cmd.sequence_number
                            );
                            commands_failed += 1;
                    ...
                    if commands_processed > 0
                        && commands_processed % STATS_LOG_INTERVAL_COMMANDS == 0
                    {
                        let (dropped_commands, timeout_count, queue_len) = ctx
                            .shared
                            .command_queue
                            .lock(|q| (q.dropped_count(), q.timeout_count(), q.len()));
                        let (dropped_responses, response_queue_len) = ctx
                            .shared
                            .response_queue
                            .lock(|q| (q.dropped_count(), q.len()));

                        log_info!(
                            "Command processing stats: processed={}, failed={}, timed_out={}",
                            commands_processed,
                            commands_failed,
                            commands_timed_out
                        );
                        log_info!(
                            "Queue stats: cmd_queue_len={}, dropped_cmds={}, timeout_cmds={}",
                            queue_len,
                            dropped_commands,
                            timeout_count
                        );
                        log_info!(
                            "Response stats: resp_queue_len={}, dropped_resp={}",
                            response_queue_len,
                            dropped_responses
                        );
    ...
    async fn bootloader_entry_task(
        mut ctx: bootloader_entry_task::Context,
        command_id: u8,
        timeout_ms: u32,
    ) {
        let current_time = get_timestamp_ms();
        ...
        let entry_result = ctx
            .shared
            .bootloader_manager
            .lock(|manager| manager.request_bootloader_entry(timeout_ms, current_time));
            ...
            Ok(()) => {
                ...
                while !entry_complete {
                    ...
                    let entry_state = ctx
                        .shared
                        .bootloader_manager
                        .lock(|manager| manager.update_entry_progress(&hardware_state, loop_time));

                    match entry_state {
                        Ok(BootloaderEntryState::ReadyForBootloader) => {
                            log_info!(
                                "Bootloader entry sequence complete, executing final shutdown"
                            );
                            ...
                            let shutdown_result = ctx
                                .shared
                                .bootloader_manager
                                .lock(|manager| manager.execute_safe_shutdown(loop_time));
    ...
    async fn test_processor_update_task(mut ctx: test_processor_update_task::Context) {
        ...
        loop {
            ...
            let test_result = ctx
                .shared
                .test_processor
                .lock(|processor| processor.update_active_test(current_timestamp));
            ...
            if let Some(result) = test_result {
                ...
                match result.status {
                    TestStatus::Completed => {
                        completed_tests += 1;
                        log_info!(
                            "Test completed: type={:?}, id={}, duration={}ms",
                            result.test_type,
                            result.test_id,
                            result.duration_ms()
                        );
                    ...
                    TestStatus::TimedOut => {
                        timed_out_tests += 1;
                        log_warn!(
                            "Test timed out: type={:?}, id={}, duration={}ms",
                            result.test_type,
                            result.test_id,
                            result.duration_ms()
                        );
                    ...
                    TestStatus::Failed => {
                        failed_tests += 1;
                        log_error!(
                            "Test failed: type={:?}, id={}, duration={}ms",
                            result.test_type,
                            result.test_id,
                            result.duration_ms()
                        );
            ...
            if update_count % STATS_LOG_INTERVAL_UPDATES == 0 {
                let stats = ctx
                    .shared
                    .test_processor
                    .lock(|processor| processor.get_statistics());

                log_info!(
                    "Test processor stats: executed={}, passed={}, failed={}",
                    stats.total_tests_executed,
                    stats.total_tests_passed,
                    stats.total_tests_failed
                );
                log_info!(
                    "Test processor update stats: completed={}, timed_out={}, failed={}",
                    completed_tests,
                    timed_out_tests,
                    failed_tests
                );
                log_info!(
                    "Success rate: {}%, Active tests: {}",
                    stats.success_rate_percent(),
                    stats.active_test_count
                );
            }
    ...
    async fn update_test_processor_timing(
        mut ctx: update_test_processor_timing::Context,
        timing_measurement: ass_easy_loop::test_processor::TimingMeasurement,
    ) {
        // Update test processor with timing measurement if pEMF timing test is active
        let result = ctx
            .shared
            .test_processor
            .lock(|processor| processor.update_pemf_timing_measurements(timing_measurement));
        ...
        loop {
            ...
            if let Some(log_message) = message_to_send {
                        ...
                        while retry_count <= MAX_RETRY_ATTEMPTS && !transmission_successful {
                            ...
                            match send_result {
                                Ok(_) => {
                                    ...
                                    {
                                        log_debug!(
                                            "Transmitted {} messages via USB HID",
                ...
                if transmission_result.is_err() {
                    ass_easy_loop::logging::record_transmission_failure();
                }

                // Record message performance statistics
                ass_easy_loop::logging::record_message_performance(
                    format_time_us,
                    0,
            ...
            if cycle_count % CPU_MEASUREMENT_INTERVAL_CYCLES == 0 {
                ...
                ass_easy_loop::logging::record_usb_cpu_usage(0, cpu_usage_percent); // Poll task CPU will be updated separately

                // Log performance statistics
                if messages_transmitted > 0 {
                    let avg_format_time_us = total_format_time_us / messages_transmitted;
                    let avg_transmission_time_us =
                        total_transmission_time_us / messages_transmitted;
            ...
            if messages_transmitted > 0 && messages_transmitted % 500 == 0 {
                ...
                let queue_memory_bytes =
                    ass_easy_loop::logging::PerformanceMonitor::calculate_queue_memory_usage::<32>(
                        queue_stats.current_utilization_percent as usize,
                    );
                let usb_buffer_memory_bytes = 1024; // Estimate for USB buffers

                // Record memory usage
                ass_easy_loop::logging::record_memory_usage(
                    queue_memory_bytes,
                    usb_buffer_memory_bytes,
                );
                ...
                if let Some(perf_stats) = ass_easy_loop::logging::get_global_performance_stats() {
    ...
    async fn system_diagnostic_task(_ctx: system_diagnostic_task::Context) {
        ...
        log_info!(
            "Monitoring intervals: diagnostic={}s, uptime={}s, memory={}s",
        ...
        loop {
            ...
            if diagnostic_cycles > 1 {
                ...
                if timing_deviation > CRITICAL_DELAY_THRESHOLD_MS {
                    log_error!(
                        "CRITICAL: System diagnostic task delayed by {}ms (cycle {})",
                        timing_deviation,
                        diagnostic_cycles
                    );
                    system_warnings += 1;
                    ...
                    log_warn!(
                        "System diagnostic task delayed by {}ms (cycle {})",
                        timing_deviation,
                        diagnostic_cycles
                    );
                    system_warnings += 1;
            ...
            if current_time - last_memory_check >= MEMORY_CHECK_INTERVAL_MS as u32 {
                ...
                if queue_utilization >= MEMORY_CRITICAL_THRESHOLD_PERCENT {
                    log_error!(
                        "CRITICAL: Log queue memory usage at {}% ({}/{} messages)",
                        queue_utilization,
                        unsafe { (*core::ptr::addr_of!(GLOBAL_LOG_QUEUE)).len() },
                        unsafe { (*core::ptr::addr_of!(GLOBAL_LOG_QUEUE)).capacity() }
                    );
                    ...
                    log_warn!(
                        "Log queue memory usage high: {}% ({}/{} messages)",
                        queue_utilization,
                        unsafe { (*core::ptr::addr_of!(GLOBAL_LOG_QUEUE)).len() },
                        unsafe { (*core::ptr::addr_of!(GLOBAL_LOG_QUEUE)).capacity() }
                    );
                ...
                log_debug!(
                    "- Queue peak utilization: {} messages",
                    queue_stats.peak_utilization
                );
                ...
                let total_estimated_memory =
                    queue_memory_bytes + estimated_stack_usage + estimated_usb_buffers;

                log_debug!(
                    "Estimated memory usage: ~{} bytes total",
            ...
            if diagnostic_cycles % 10 == 0 {
                ...
                if queue_stats.messages_dropped > 0 {
                    log_warn!(
                        "System diagnostic: {} log messages dropped due to queue overflow",
                ...
                match leak_result {
                    ass_easy_loop::resource_management::LeakDetectionResult::NoLeaks => {
                        log_info!("Resource leak detection: No leaks detected");
                    }
                    ass_easy_loop::resource_management::LeakDetectionResult::PotentialLeaks(
                        leaks,
                    ) => {
                        log_warn!(
                            "Resource leak detection: {} potential issues found",
                            leaks.len()
                        );
                ...
                if system_warnings > 50 {
                    log_error!(
                        "SYSTEM STABILITY WARNING: {} warnings in {} cycles",
                        system_warnings,
                        diagnostic_cycles
                    );
                } else if system_warnings > 20 {
                    log_warn!(
                        "System stability notice: {} warnings in {} cycles",
                        system_warnings,
                        diagnostic_cycles
                    );
                }
            ...
            if current_time - last_uptime_log >= UPTIME_LOG_INTERVAL_MS as u32 {
                ...
                log_info!(
                    "- Average diagnostic cycle time: {}ms",
                    if diagnostic_cycles > 0 {
                        current_time / diagnostic_cycles
                    } else {
                        0
                    }
                );
                ...
                let efficiency_percent =
                    if queue_stats.messages_sent + queue_stats.messages_dropped > 0 {
                        (queue_stats.messages_sent * 100)
                            / (queue_stats.messages_sent + queue_stats.messages_dropped)
                    } else {
                        100
                    };
                log_info!(
                    "- System efficiency: {}% (messages delivered vs total)",
                    efficiency_percent
                );
    ...
    async fn performance_benchmark_task(_ctx: performance_benchmark_task::Context) {
        ...
        log_info!(
            "Benchmark interval: {}s, Timing samples: {}",
            BENCHMARK_INTERVAL_MS / 1000,
            TIMING_MEASUREMENT_SAMPLES
        );
        ...
        log_info!(
            "Baseline established: pEMF={}us, Battery={}us",
        ...
        loop {
            ...
            let battery_deviation_us =
                current_battery_timing_us.abs_diff(baseline_battery_timing_us);

            // Record timing impact measurements
            ass_easy_loop::logging::record_timing_impact(pemf_deviation_us, battery_deviation_us);
            ...
            log_info!(
                "pEMF timing: baseline={}us, current={}us, deviation={}us ({}%)",
                baseline_pemf_timing_us,
                current_pemf_timing_us,
                pemf_deviation_us,
                pemf_impact_percent
            );
            log_info!(
                "Battery timing: baseline={}us, current={}us, deviation={}us ({}%)",
                baseline_battery_timing_us,
                current_battery_timing_us,
                battery_deviation_us,
            ...
            if pemf_impact_percent > TIMING_TOLERANCE_PERCENT {
                log_warn!(
                    "pEMF timing impact exceeds tolerance: {}% > {}%",
                    pemf_impact_percent,
            ...
            if battery_impact_percent > TIMING_TOLERANCE_PERCENT {
                log_warn!(
                    "Battery timing impact exceeds tolerance: {}% > {}%",
                    battery_impact_percent,
                    TIMING_TOLERANCE_PERCENT
                );
            }
            ...
            if let Some(perf_stats) = ass_easy_loop::logging::get_global_performance_stats() {
                ...
                log_info!(
                    "  Queue: {}KB, USB Buffers: {}KB, Total: {}KB ({}%)",
                ...
                log_info!(
                    "  Avg Format: {}us, Avg TX: {}us, Peak: {}us",
                ...
                log_info!(
                    "  pEMF Deviation: {}us, Battery Deviation: {}us, Max: {}us",
...
pub fn validate_led_control_logic() -> bool {
    // BatteryState is imported from ass_easy_loop at module level
    ...
    if low_state == normal_state {
        return false;
    }
    if normal_state == charging_state {
        return false;
    }
    if charging_state == low_state {
        return false;
    }
